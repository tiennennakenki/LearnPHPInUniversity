...
FROM sua CROSS JOIN hang_sua CROSS JOIN loai_sua WHERE `sua`.`Ma_hang_sua`=`hang_sua`.`Ma_hang_sua` and `sua`.`Ma_loai_sua`= 'SB'  and `sua`.`Ma_loai_sua`= `loai_sua`.`Ma_loai_sua` LIMIT $offset, $rowsPerPage;";
...
if($numRows<>0)
{
    ...
    echo"</table> </div>";
    $re = mysqli_query($conn, 'select * from sua where sua.Ma_loai_sua = "SB"');
